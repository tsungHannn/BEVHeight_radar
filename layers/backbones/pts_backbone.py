...
class PtsBackbone(nn.Module):
    ...
    def _forward_single_sweep(self, pts):
        ...
        voxels, num_points, coors = self.voxelize(pts) # Voxelize
        if self.times is not None:
            t2.record()
            torch.cuda.synchronize()
            self.times['pts_voxelize'].append(t1.elapsed_time(t2))

        voxel_features = self.pts_voxel_encoder(voxels, num_points, coors) # 用 PillarFeatureNet
        x = self.pts_middle_encoder(voxel_features, coors, batch_size) # 用 PillarFeatureNet 建構 Stacked Pillars 後，投影成 Pseudo Images
        x = self.pts_backbone(x) # 用 SECOND 提取特徵 -> 論文的F_R
