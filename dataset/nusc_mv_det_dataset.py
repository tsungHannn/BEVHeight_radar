...
# 模擬雷達數據
#####################################################
# def generate_radar_data(batch_size, num_sweeps,num_cams, num_points, num_features):
#     data = torch.rand((batch_size, num_sweeps, num_cams, num_points, num_features))
#     labels = np.random.randint(0, 2, (batch_size,)) # 隨機生成0或1的標籤
#     return data, labels

# radarData, radarLabel = generate_radar_data(1, 1, 1, 2, 10)
# radarData = torch.tensor(radarData, dtype=torch.float32)
# radarLabel = torch.tensor(radarLabel, dtype=torch.long)


# (batch_size, num_sweeps, num_cams, num_points, num_features)
# radarData = torch.rand((2, 1, 1, 1000, 6))
######################################################
        ...
        for cam in cams:
            ...
            for sweep_idx, cam_info in enumerate(cam_infos):
                ...
                if self.return_radar_pv:    
                    ...
                    pc = pypcd.PointCloud.from_path(os.path.join(
                        self.data_root, self.radar_pv_path, f'{file_name}.pcd'))
                    ...
                    np_rcs = np.zeros_like(np_x)
                    np_dummy = np.zeros_like(np_x)
                    points_32 = np.transpose(np.vstack((np_x, np_y, np_z, np_rcs, np_vx, np_vy, np_dummy)))
                    # 原本是 [x, y, z, rcs, vx_comp, vy_comp, dummy_field]
...
def collate_fn(data, is_return_depth=False, is_return_radar_pv=True):
    imgs_batch = list()
    ...
    # np.savez("/john/mats_dict.npz", **mats_dict)
