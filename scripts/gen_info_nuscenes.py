...
def main():
    trainval_nusc = NuScenes(version='v1.0-trainval',
                             dataroot='/HDD/nuscenes/',
                             verbose=True)
    train_scenes = splits.train
    val_scenes = splits.val
    train_infos_tiny = generate_info(trainval_nusc, train_scenes[:2])
    mmcv.dump(train_infos_tiny, '/HDD/nuscenes/nuscenes_infos_train-tiny.pkl')
    train_infos = generate_info(trainval_nusc, train_scenes)
    mmcv.dump(train_infos, '/HDD/nuscenes/nuscenes_infos_train.pkl')
    val_infos = generate_info(trainval_nusc, val_scenes)
    mmcv.dump(val_infos, '/HDD/nuscenes/nuscenes_infos_val.pkl')
